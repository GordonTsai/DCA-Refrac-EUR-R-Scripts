...
#production 
production = read.csv("DCA_production2.csv", header = FALSE)
...
minProduction <- 50.0
#taildecline <- new(J("java.util.String"))
#J(taildecline,"add","Exponential")
...
#.jcall(DCAdriver, "V", "setMinimumProduction", 25)
J( DCAdriver, "model")
failure = .jcall(DCAdriver,"S", "getModelFailure")

report = .jcall(DCAdriver, "S", "getReport")

DCAeur <- J( DCAdriver, "getEUR")
CumulativeProduction <- J(DCAdriver, "getCumulativeProduction")
EstimatedReserves <- J(DCAdriver, "getEstimatedReserves")
